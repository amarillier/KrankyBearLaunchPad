...
echo "KrankyBear LaunchPad - Windows Compile (bash)"
echo "================================================"
echo
...
rm -f bin/launchpad-windows.exe || true
...
if ! go build -ldflags="$LDFLAGS" -trimpath -o bin/launchpad-windows.exe
then
...
ls -lh bin/launchpad-windows.exe || true
...
then
  ...
  cp -f bin/launchpad-windows.exe ./KrankyBearLaunchPad.exe
  ISCC_DEFAULT="/c/Program Files (x86)/Inno Setup 6/ISCC.exe"
  if [[ -x "$ISCC_DEFAULT" || -f "$ISCC_DEFAULT" ]]
  then
    "${ISCC_DEFAULT}" ./Inno/KrankyBearLaunchPad.iss || {
